...
func TestServiceWorkflow(t *testing.T) {
	service := New()
	if _, err := service.Save("alice", model.Payment{ID: "one", Name: "First", Status: "ready", Priority: 2}); err != nil {
		t.Fatal(err)
	}
	snapshot := service.Capture("before batch")
	err := service.Apply("bob", []batch.Operation{
		{Kind: batch.Upsert, Value: model.Payment{ID: "one", Name: "Updated", Status: "ready", Priority: 3}},
		{Kind: batch.Upsert, Value: model.Payment{ID: "two", Name: "Second", Status: "new", Priority: 1}},
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestServiceRejectsInvalidOperations(t *testing.T) {
	service := New()
	if _, err := service.Save("alice", model.Payment{}); err == nil {
