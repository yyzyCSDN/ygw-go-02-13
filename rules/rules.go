...
type Predicate interface {
	Match(model.Payment) bool
	Describe() string
}

type PredicateFunc struct {
	Name string
	Test func(model.Payment) bool
}

func (predicate PredicateFunc) Match(value model.Payment) bool {
...
func (predicates All) Match(value model.Payment) bool {
...
func (predicates Any) Match(value model.Payment) bool {
	for _, predicate := range predicates {
		if predicate != nil && predicate.Match(value) {
			return true
		}
...
func (predicate Not) Match(value model.Payment) bool {
...
func Active() Predicate {
	return PredicateFunc{Name: "active", Test: func(value model.Payment) bool { return value.Active }}
}

func Status(status string) Predicate {
	want := strings.ToLower(strings.TrimSpace(status))
	return PredicateFunc{Name: "status=" + want, Test: func(value model.Payment) bool {
...
func Tagged(tag string) Predicate {
	...
	return PredicateFunc{Name: "tag=" + want, Test: func(value model.Payment) bool { return value.HasTag(want) }}
}

func MinimumAmount(minimum int64) Predicate {
	return PredicateFunc{Name: fmt.Sprintf("amount>=%d", minimum), Test: func(value model.Payment) bool {
		return value.Amount >= minimum
	}}
}

func PriorityBetween(minimum, maximum int) Predicate {
	return PredicateFunc{Name: fmt.Sprintf("priority=%d..%d", minimum, maximum), Test: func(value model.Payment) bool {
...
func (set *Set) Evaluate(value model.Payment) Evaluation {
...
func (set *Set) Select(values []model.Payment, name string) ([]model.Payment, error) {
	if set == nil {
