...
type Result struct {
	Values  []model.Payment
	Created int
	Updated int
	Deleted int
}

func Apply(current []model.Payment, operations []Operation) (Result, error) {
	working := map[string]model.Payment{}
	...
	for index, operation := range operations {
		...
		case Delete:
			if _, exists := working[operation.ID]; !exists {
				return Result{}, fmt.Errorf("operation %d: payment %q does not exist", index+1, operation.ID)
			}
	...
	sort.Strings(ids)
	result.Values = make([]model.Payment, 0, len(ids))
	for _, id := range ids {
...
func ValidateUnique(values []model.Payment) error {
