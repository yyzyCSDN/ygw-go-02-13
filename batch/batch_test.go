...
func TestApplyOperations(t *testing.T) {
	current := []model.Payment{{ID: "one", Name: "One"}, {ID: "two", Name: "Two"}}
	result, err := Apply(current, []Operation{
		{Kind: Upsert, Value: model.Payment{ID: "one", Name: "Updated"}},
		{Kind: Delete, ID: "two"},
		{Kind: Upsert, Value: model.Payment{ID: "three", Name: "Three"}},
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestApplyFailureReturnsNoPartialResult(t *testing.T) {
	result, err := Apply([]model.Payment{{ID: "one", Name: "One"}}, []Operation{
		{Kind: Upsert, Value: model.Payment{ID: "two", Name: "Two"}},
	...
	if err := ValidateUnique([]model.Payment{{ID: "x"}, {ID: "x"}}); err == nil {
