...
type Store struct {
	...
	revision uint64
	items    map[string]model.Payment
	history  []Change
	limit    int
}
...
func New(historyLimit int) *Store {
	...
	return &Store{clock: time.Now, items: map[string]model.Payment{}, limit: historyLimit}
}

func (store *Store) Put(value model.Payment) (model.Payment, error) {
...
func (store *Store) Create(value model.Payment) (model.Payment, error) {
	store.mu.RLock()
	_, exists := store.items[value.ID]
	store.mu.RUnlock()
	if exists {
		return model.Payment{}, fmt.Errorf("payment %q already exists", value.ID)
	}
	return store.Put(value)
}

func (store *Store) Get(id string) (model.Payment, bool) {
	store.mu.RLock()
...
func (store *Store) Delete(id string) (model.Payment, error) {
	store.mu.Lock()
	defer store.mu.Unlock()
	value, exists := store.items[id]
	if !exists {
		return model.Payment{}, fmt.Errorf("payment %q does not exist", id)
	}
...
func (store *Store) List() []model.Payment {
	store.mu.RLock()
	defer store.mu.RUnlock()
	result := make([]model.Payment, 0, len(store.items))
...
func (store *Store) FindByName(text string) []model.Payment {
...
func (store *Store) ReplaceAll(values []model.Payment) error {
	replacement := make(map[string]model.Payment, len(values))
	now := store.clock()
	for _, value := range values {
		...
		if _, duplicate := replacement[value.ID]; duplicate {
			return fmt.Errorf("duplicate payment id %q", value.ID)
		}
