...
func EncodeJSON(values []model.Payment) ([]byte, error) {
	cloned := make([]model.Payment, len(values))
	for index, value := range values {
		cloned[index] = value.Clone()
	}
	data, err := json.MarshalIndent(cloned, "", "  ")
	if err != nil {
		return nil, fmt.Errorf("encode payment list: %w", err)
	}
	return append(data, '\n'), nil
}

func DecodeJSON(reader io.Reader) ([]model.Payment, error) {
	decoder := json.NewDecoder(reader)
	decoder.DisallowUnknownFields()
	var values []model.Payment
	if err := decoder.Decode(&values); err != nil {
		return nil, fmt.Errorf("decode payment list: %w", err)
	}
	for index := range values {
		if err := values[index].Validate(); err != nil {
			return nil, fmt.Errorf("payment %d: %w", index+1, err)
		}
...
func EncodeCSV(values []model.Payment) ([]byte, error) {
	...
	for _, value := range values {
		...
		if err := writer.Write(row); err != nil {
			return nil, fmt.Errorf("write payment CSV: %w", err)
		}
	}
	writer.Flush()
	if err := writer.Error(); err != nil {
		return nil, fmt.Errorf("flush payment CSV: %w", err)
	}
	return buffer.Bytes(), nil
}

func DecodeCSV(reader io.Reader) ([]model.Payment, error) {
	rows, err := csv.NewReader(reader).ReadAll()
	if err != nil {
		return nil, fmt.Errorf("read payment CSV: %w", err)
	}
	if len(rows) == 0 {
	...
	result := make([]model.Payment, 0, len(rows)-1)
	for rowNumber, row := range rows[1:] {
		...
		value := model.Payment{ID: row[0], Name: row[1], Status: row[2], Priority: priority, Amount: amount, Active: active, Version: version, Tags: strings.Split(row[7], "|"), UpdatedAt: updated}
