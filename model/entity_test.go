...
func TestPaymentCloneAndTags(t *testing.T) {
	original := Payment{ID: "one", Name: "First", Tags: []string{"Blue"}, Metadata: map[string]string{"zone": "a"}}
...
func TestPaymentValidationAndTouch(t *testing.T) {
	if err := (Payment{}).Validate(); err == nil {
		t.Fatal("expected validation error")
	}
	now := time.Unix(100, 0).UTC()
	value := (Payment{ID: "one", Name: "First"}).Touch(now)
