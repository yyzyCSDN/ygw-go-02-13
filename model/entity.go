...
// Payment is the canonical record managed by Payment Ledger.
type Payment struct {
	ID        string            `json:"id"`
...
func (value Payment) Clone() Payment {
	value.Tags = append([]string(nil), value.Tags...)
	value.Metadata = cloneMetadata(value.Metadata)
	return value
}

func (value Payment) Validate() error {
	if strings.TrimSpace(value.ID) == "" {
		return fmt.Errorf("payment id is empty")
	}
	if strings.TrimSpace(value.Name) == "" {
		return fmt.Errorf("payment name is empty")
	}
	if value.Priority < 0 {
		return fmt.Errorf("payment priority cannot be negative")
	}
	if value.Amount < 0 {
	...
	for key := range value.Metadata {
		if strings.TrimSpace(key) == "" {
			return fmt.Errorf("payment metadata contains a blank key")
		}
	}
	return nil
}

func (value Payment) HasTag(tag string) bool {
...
func (value Payment) MetadataValue(key string) (string, bool) {
	result, ok := value.Metadata[key]
	return result, ok
}

func (value Payment) WithTag(tag string) Payment {
	copy := value.Clone()
	tag = normalizeToken(tag)
...
func (value Payment) WithoutTag(tag string) Payment {
	copy := value.Clone()
	tag = normalizeToken(tag)
...
func (value Payment) WithMetadata(key, content string) Payment {
	copy := value.Clone()
...
func (value Payment) Touch(now time.Time) Payment {
	copy := value.Clone()
